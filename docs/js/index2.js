...
async function SendData(JWTToken){
    ...
    if (JWTToken.value == ''){
        ...
        dataGraphQL = await getData(JWTToken)
         ...
         if (show !== null){
         show.addEventListener("click", () => charts.AuditData(dataGraphQL.auditedPeople, true))
         }
...
function userdata(allUserData){
    ...
    <h4>Email: ${allUserData.attrs.email}</h4>
    <h4>Username: ${allUserData.login}</h4>
    <h4>Created At: ${functions.splitString(allUserData.createdAt, 0, 10)}</h4>
    <h4>Audit Ratio: ${allUserData.auditRatio.toFixed(3)}</h4>`
}

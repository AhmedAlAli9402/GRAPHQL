export function BarGraph(pointsPerMonth){
  ...
  <svg version="1.1" xmlns:xlink="http://www.w3.org/1999/xlink" class="chart" width="620" height="520" aria-labelledby="title asc" role="img">
  ...
  while(i < 12) {
    ...
    <text x="${x}" y="200">${pointsPerMonth[i]/1000}KB</text></g>`
    if (i === 11){
      i = 0
    } else {
      i++
...
<h4>audits passed ≈${Math.round(passedPer)}%  (${done-failed} audits)</h4><i class="gg-border-all" style="color:green; background-color:green; margin-top:-2.5rem"></i>
  ...
  <h4>audits missed ≈${Math.round(missedPer)}%  (${missed} audits)</h4><i class="gg-border-all" style="color:grey; background-color:grey; margin-top:-2.5rem"></i>`
}
